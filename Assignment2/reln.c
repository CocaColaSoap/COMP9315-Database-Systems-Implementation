...
PageID addToRelation(Reln r, Tuple t)
{
	...
	if ((r->insertNumber) >= c)
	{
		Page np1 = newPage();
		Page np2 = newPage();
		Page pg = getPage(dataFile(r), r->sp);
		Offset ovg = pageOvflow(pg);
		char *data = pageData(pg);
		pageSetOvflow(np1, ovg);
		putPage(r->data, r->sp, np1);
		np1 = getPage(dataFile(r), r->sp);
		putPage(r->data, r->npages, np2);
		np2 = getPage(dataFile(r), r->npages);	
		while (strlen(data) != 0)
		{	
			int l = strlen(data);
			h = tupleHash(r, data);
	...
	if (addToPage(pg, t) == OK)
	{
		putPage(r->data, p, pg);
		r->ntups++;
		r->insertNumber = r->insertNumber + 1;
		return p;
	}
